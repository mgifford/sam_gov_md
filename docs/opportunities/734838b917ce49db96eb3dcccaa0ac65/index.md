---
layout: default
title: Battery Power Supply
agency: DEPT OF DEFENSE
notice_type: Solicitation
notice_id: 734838b917ce49db96eb3dcccaa0ac65
---

# Battery Power Supply

- Agency: DEPT OF DEFENSE
- Type: Solicitation
- Posted: 2026-03-04 16:07:32.692-05
- Solicitation Number: SPRDL1-25-R-0093

## Summary

THIS IS A SOLICITATION NOTICE. This announcement is for the following requirement - Item: Battery Power Supply. NSN: 6130-01-604-1361. Part Number: 13022799. Basic Quantity: 80 Each. Option Quantity: Up to 80 Each. FOB Point: Destination. Packaging: MIL B. Type Award: Firm-Fixed-Price. The Technical Date Package (TDP) is considered competitive with purchase parts and is Export Controlled. Access to the TDP will require you to complete an attachment titled "NDA for Government Protected Information". Please return the complete and signed document "NDA for Government Protected Information" to the primary point of contact email listed below for review. Access will be provided if the review is approved. Please note: ONLY the Data Custodian, as identified in the United States/Canada Joint Certification Program (JCP) will be granted approval to download the Technical Data Package (TDP), once it is made available in Contract Opportunities �- No Exceptions. See the solicitation for the actual closing date and the full terms and conditions. The solicitation can be downloaded from the Contract Opportunities section of SAM.gov. If you do not already have access to SAM.gov, it is recommended you register as soon as possible.

## Contacts

- Primary Contact:
  - Name: Abhinav Dang
  - Email: abhinav.dang@dla.mil
- Secondary Contact:
  - Name: DLA Warren Contract Opportunities
  - Email: dlawarrencontractopportunities@dla.mil

## Attachments

**Total: 1 attachment(s)**

- Attachment 1: "NDA for Government Protected Information". Please return the complete and signed document "NDA for Government Protected Information" to the primary point of contact email listed below for review. Access will be provided if the review is approved. Please note: ONLY the Data Custodian, as identified in the United States/Canada Joint Certification Program (JCP) will be granted approval to download the Technical Data Package (TDP), once it is made available in Contract Opportunities �- No Exceptions. See the solicitation for the actual closing date and the full terms and conditions. The solicitation can be downloaded from the Contract Opportunities section of SAM.gov. If you do not already have access to SAM.gov, it is recommended you register as soon as possible

## Links

- [SAM.gov opportunity page](https://sam.gov/workspace/contract/opp/734838b917ce49db96eb3dcccaa0ac65/view)