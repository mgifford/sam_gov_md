...
def extract_officers():
    ...
    officer_stats = defaultdict(lambda: {
        "email": None,
        "phone": None,
        "departments": set(),
        "opportunities": 0,
    ...
    dept_officers = defaultdict(lambda: defaultdict(lambda: {
        "opportunities": 0,
        "awards": 0,
        "total_award_value": 0,
    }))
    ...
    for record in records:
        ...
        if primary_name and primary_name != "":
        ...
        if secondary_name and secondary_name != "":
    ...
    for officer in officer_stats.values():
        officer["departments"] = sorted(list(officer["departments"]))
    ...
    # Output results
    output = {
        "extraction_date": Path("docs/data/today_summary.json").read_text() 
            and json.loads(Path("docs/data/today_summary.json").read_text()).get("effective_date"),
    ...
    output_path = Path("docs/data/contract_officers.json")
    output_path.write_text(json.dumps(output, indent=2))
    
    print(f"✅ Extracted {len(officer_stats)} unique contract officers")
    print(f"📊 Top 3 Most Active Officers:")
    ...
    print(f"\n📁 Results saved to docs/data/contract_officers.json")
